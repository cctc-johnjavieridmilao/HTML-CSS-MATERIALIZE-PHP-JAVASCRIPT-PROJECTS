...
$results = mysqli_query($connect,"SELECT COUNT(*) FROM children");
...
<?php include 'sidebar.php'; ?>
  ...
  <?php
    include 'connection/mysqliconn.php';  
    if (isset($_POST['btn_approve_comm'])) {
       $id = $_GET['id'];
       $query = "SELECT stat FROM children WHERE id = '$id'";
       $res = mysqli_query($connect,$query);
       $row = mysqli_fetch_assoc($res);
       $stat = $row['stat'];
       if ($stat == 0) {
         $queryUpdate = "UPDATE children SET stat=1 WHERE id='$id'";
         $execute = mysqli_query($connect,$queryUpdate);
         echo "<script>alert('Comment Approve');</script>";
       }
       else{
        echo "<script>alert('Comments Alredy Approve');</script>";
...
<div id="view" class="modal fade">  
      <div class="modal-dialog">  
           <div class="modal-content">  
                ...
                <h4 class="modal-title" id="com_name" style="font-size: 18px;color: white;"> <span class="fas fa-sign-out-alt"> </span></h4>  
                </div>
...
$(document).ready(function() {
  ...
  $("#resulta").load("fetch_paginate_cm.php").fadeIn('slow');
  $(".pagination").bootpag({
    ...
    if(true){
      ...
      $("#resulta").hide();
       setTimeout(function(){
        $("#resul").hide();
     $("#resulta").fadeOut("slow").load("fetch_paginate_cm.php", {'page':num}).fadeIn(1500);
    },2000);
    }
