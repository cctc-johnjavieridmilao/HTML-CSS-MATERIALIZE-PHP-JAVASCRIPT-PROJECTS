...
<script>
  $(document).ready(function(){
    $('#form_submitSS').on('submit', function(event){
    event.preventDefault();
    var timer = 2000;
     if ($('#uname').val() == "") {
      swal ('Username is required','','warning');
     }
     else if ($('#new').val() == "") {
      swal ('Newpass is required','','warning');
     }
     else if ($('#confirm').val() == "") {
      swal ('Confirmpass is required','','warning');
     }
      else if ($('#confirm').val() != $('#new').val()) {
      swal ('Password do not Match is required','','warning');
     }
     else{
     $.ajax({
     url: 'update_ad_acc.php',
     method: 'POST',
     data:$('#form_submitSS').serialize(),
     cache:false,
     beforeSend:function() {
     $('#spin_spin').show();
  ...
  $(document).ready(function(){
    ...
    $('#load_table').load('load_admin_table.php');
    }, 1000);
  });
</script>
<script>
...
<script>
  ...
  function readProducts(){
    $('#load-products').load('message_user.php'); 
  }
