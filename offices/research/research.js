$(document).ready(function () {
    ...
    //                 Move date : ${responce.move_at_r}<br>
    //                 Move by : ${responce.move_at_r}
    //                 </p>
    //                 <a href="#" id="${responce.files_id}" class="secondary-content move"><i class="material-icons">arrow_forward</i></a>
    //                 </li>
    //             </ul>
    //                 `
    //                 }
            ...
            success: function (data) {
                ...
                setTimeout(() => {
                    if (res.success === 'yes') {
                        loader.innerHTML = '';
                        M.toast({
                            html: '<span>File Move to Ara office</span>'
                        })
                        getdatas();
                    } else {
                        loader.innerHTML = '';
        ...
        $.get('../../php/check_move_r.php', function (data) {
            ...
            res.forEach(respo => {
                if (respo.acc_username === 'faculty' && respo.file_type === 'Personal Data Sheet') {
                    ...
                    if (respo.files_id === respo.id && respo.fileid === null && respo.file_id === null) {
                        output += `
                    <ul class="collection">
                    <li class="collection-item avatar">
                    <i class="material-icons circle">folder</i>
                    ...
                    Move date : ${respo.move_at_r}<br>
                    Move by : ${respo.move_by_r}
                    </p>
                    <a href="#" id="${respo.files_id}" class="secondary-content move"><i class="material-icons">arrow_forward</i></a>
                    </li>
                    </ul>
                    `;
                        document.getElementById('check_all_move_text').textContent = 'All Files Are Move';
                    } else if (respo.files_id === respo.id && respo.fileid === respo.files_id) {
                        output2 += `
                        <ul class="collection">
                        <li class="collection-item avatar">
                        <i class="material-icons circle">folder</i>
                        ...
                        `;
                        document.getElementById('check_all_move_text').textContent = '';
                    }
                }
                document.getElementById('result').innerHTML = output;
